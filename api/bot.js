...
if (bot) {
  const checkAccessMiddleware = async (ctx, next) => {
    ...
    const sent = await ctx.reply(
      '⛔ <b>AKSES DITOLAK</b>\n\nBot ini bersifat privat. Silakan klik tombol di bawah untuk meminta akses.',
      {
        parse_mode: 'HTML',
  ...
  bot.action(/^acc_user_(.+)$/, async (ctx) => {
    ...
    try {
      ...
      if (res.data && res.data.status === 'success') {
        await ctx.editMessageText(`✅ <b>SUKSES!</b> User ID <code>${targetId}</code> berhasil di-ACC ke Spreadsheet.`, { parse_mode: 'HTML' });
        try {
          await ctx.telegram.sendMessage(targetId, '🎉 <b>Akses disetujui!</b>\n\nKetik /start untuk menggunakan bot.', { parse_mode: 'HTML' });
        } catch (e) {}
      } else {
        await ctx.reply(`❌ Gagal di GAS: ${JSON.stringify(res.data)}`);
      }
    } catch (err) {
      await ctx.reply(`🚨 Error koneksi: ${err.message}`);
    }
  });
  ...
  bot.action('start_blast', async (ctx) => {
    ...
    await clearBotMsg(ctx, session);
    const sent = await ctx.reply('📧 <b>Kirim daftar email target (pisahkan dengan koma atau baris baru):</b>', { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('❌ Batalkan', 'cancel')]]) });
    session.lastMsgId = sent.message_id;
  });
  ...
  bot.action('stop_blast', async (ctx) => {
    ...
    try {
      ...
      const sent = await ctx.reply(`🛑 <b>SINYAL STOP DIKIRIM</b>\n\n${response.data.message || 'Berhasil.'}`, { parse_mode: 'HTML', ...mainMenu });
      session.lastMsgId = sent.message_id;
    } catch (err) {
  ...
  bot.action('check_quota', async (ctx) => {
    ...
    try {
      ...
      const sent = await ctx.reply(`🔋 <b>KUOTA GMAIL</b>\n\nSisa hari ini: <b>${response.data.quota !== undefined ? response.data.quota : '0'} email</b>`, { parse_mode: 'HTML', ...mainMenu });
      session.lastMsgId = sent.message_id;
    } catch (err) {
      const sent = await ctx.reply(`🚨 Gagal: ${err.message}`, mainMenu);
      session.lastMsgId = sent.message_id;
  ...
  bot.on('text', async (ctx) => {
    ...
    switch (session.step) {
      ...
      case 'AWAIT_RECIPIENTS_TEXT':
        ...
        if (emails.length === 0) {
          await clearBotMsg(ctx, session);
          const sent = await ctx.reply('❌ Tidak ada email valid yang ditemukan. Coba lagi:', Markup.inlineKeyboard([[Markup.button.callback('❌ Batalkan', 'cancel')]]));
          session.lastMsgId = sent.message_id;
          return;
        }
...
module.exports = async (req, res) => {
  try {
    if (!BOT_TOKEN) {
      return res.status(200).send('Bot Active! (Warning: BOT_TOKEN belum diset di Environment Vercel)');
    }
    if (req.method === 'POST') {
      if (req.body && bot) await bot.handleUpdate(req.body);
      return res.status(200).send('OK');
    } else {
      return res.status(200).send('Bot Active & Running!');
    }
  } catch (err) {
    return res.status(200).send('Handled Error: ' + err.message);
  }
};
